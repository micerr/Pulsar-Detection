...
if __name__ == "__main__":
    ...
    # Print Calibrate Score for each application and target application
    scores = [slr, ssvm]
    print("Calibration")
    for i in range(2):
        for effPrio in effPriors:
    ...
    # Print Bayes Cost Function for each Target, then we will choose only 0.5
    for effPrio in effPriors:
        lr.setPiT(effPrio)
        sclr = cv.fit(slr, LTR)
    ...
    print_DCFs([sclr, scsvm, sFusion], LTR, ["Quad Log Reg", "SVM RBF", "Fusion"], "fusion_Cost_Function", "Bayes error plot for the fused system")

    print_DETs([sclr, scsvm, sFusion], LTR, ["Quad Log Reg", "SVM RBF", "Fusion"], "fusion")
    print_ROCs([sclr, scsvm, sFusion], LTR, ["Quad Log Reg", "SVM RBF", "Fusion"], "fusion")
